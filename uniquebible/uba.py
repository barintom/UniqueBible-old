...
def main():
    ...
    if thisOS == "Windows":
        ...
        icon_path = os.path.abspath(
            os.path.join(sys.path[0], "htmlResources", "UniqueBibleApp.ico")
        )
    ...
    if not runMode.lower() in (
        ...
        if thisOS == "Windows":
            ...
            shortcutCommand1 = (
                f'''powershell.exe -NoExit -Command "python '{thisFile}'"'''
            )
            # terminal mode shortcut
            shortcutBat2 = os.path.join(shortcutDir, "UniqueBibleAppTerminal.bat")
            shortcutCommand2 = (
                f'''powershell.exe -NoExit -Command "python '{thisFile}' terminal"'''
            )
            windowsShortcuts = {
        ...
        # On non-Windows platforms
            ...
            if not os.path.exists(shortcutSh):
                ...
                with open(shortcutSh, "w") as fileObj:
                    fileObj.write(
                        "#!{0}\n{1} {2} gui".format(
                            os.environ["SHELL"], sys.executable, thisFile
                        )
                    )
                # Set permission
                for file in (
        ...
        if thisOS == "macOS" and os.path.isdir("~/Desktop/"):
            ...
            if not os.path.isfile(shortcut_file):
                ...
                appFile = "uba.py"
                icon_path = os.path.abspath(
                    os.path.join("htmlResources", f"{app}.icns")
                )
...
""".format(wd, sys.executable, thisFile, iconPath)
            ...
            if not os.path.exists(ubaLinuxDesktopFile):
                ...
                try:
                    ...
                    userAppDirShortcut = os.path.join(
                        userAppDir, "UniqueBibleApp.desktop"
                    )
                    ...
                    desktopPathShortcut = os.path.join(
                        desktopPath, "UniqueBibleApp.desktop"
                    )
                    if os.path.exists(desktopPath) and not os.path.exists(
                        desktopPathShortcut
                    ):
    ...
    if enableCli:
        ...
        cmd = [python, "-m", mainModule]
        if initialCommand:
            cmd.append(initialCommand)
